...
use_ok 'Bio::EnsEMBL::Xref::Parser::ZFIN::RefSeqParser';
...
# Create RefSeq xref to which ZFIN test data refers
...
throws_ok {
  Bio::EnsEMBL::Xref::Parser::ZFIN::RefSeqParser->new(
    species_id => $species_id,
    files      => [ "$Bin/test-data/zfin/refseq.txt" ],
...
isa_ok( $parser, 'Bio::EnsEMBL::Xref::Parser::ZFIN::RefSeqParser' );

$parser->run();

# test ZFIN dependent xref has been assigned to refseq xref above
my $hit = $db->schema->resultset( 'DependentXref' )->fetch_dependent_xref( $refseq_xref->accession, 'ZDB-GENE-040426-2161' );
